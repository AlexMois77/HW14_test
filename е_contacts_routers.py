...
def test_get_contacts(client: TestClient, auth_headers, test_user, test_user_contact: Contact):
...
def test_get_contacts_all(client: TestClient, auth_headers, test_user, test_user_contact: Contact):
...
def test_search_contacts(client: TestClient, auth_headers, test_user, test_user_contact: Contact):
    response = client.get(f"/contacts/search/?query={test_user_contact.email}", headers=auth_headers)
    assert response.status_code == status.HTTP_200_OK
    contacts = response.json()
    assert len(contacts) == 1
    assert contacts[0]["email"] == test_user_contact.email
...
def test_get_upcoming_birthdays(client: TestClient, auth_headers, test_user, test_user_contact: Contact):
